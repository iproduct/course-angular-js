...
export interface TodoFilter {
	(items: TodoItem[], query: TodoItemData): TodoItem[];
}
...
export class TodoCtrl {
	...
	constructor(
		...
		private filterFilter: TodoFilter
	) {
	...
	addTodo(): void {
		...
		this.todos.push(new TodoItem(0, newTodo, false));
		this.$scope.newTodo = '';
	}
	...
	revertEdits(todoItem: TodoItem): void {
	...
	clearDoneTodos(): void {
		this.$scope.todos = this.todos = this.todos.filter((todoItem: TodoItem) => !todoItem.completed);
	}
